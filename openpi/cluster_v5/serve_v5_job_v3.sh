...
# v3 (2026-09-06 22:15): SERVE_FORCE_SUBTASK="<sentence>" is passed QUOTED (SERVE_EXTRA is word-split, so a sentence with spaces
# cannot travel through it). Otherwise identical to v2.
...
srun --jobid="$JOB" --overlap --nodes=1 --ntasks=1 --cpus-per-task=4 --gres=gpu:"$GRES" \
  ...
  .venv/bin/python -u scripts/serve_yam_memory.py --dir "$ck" --config "$cfg" --port "$port" --warmup ${SERVE_EXTRA:-} ${SERVE_FORCE_SUBTASK:+--force-subtask "$SERVE_FORCE_SUBTASK"} 2>&1 | tee -a "$log"  # SERVE_EXTRA="--num-steps 6" etc.
