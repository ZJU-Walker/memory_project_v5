...
# targeting the SINGLE H200 job 17267793 (user 19:51: the B6 continuation keeps the 2xH200 pair; all A6 tests go
# here). GPUS=1 BATCH=2 -- the H200 fits batch 2 next to the A6sd tray probe; note the global batch differs from the
# baselines (A6 batch 4 on 2 GPUs, A6sd batch 8 on 4), which the separation term itself does not depend on.
...
echo "queue14b (A6sep: sentence-separation loss w=1.0 margin 0.3, single H200 job 17267793, user 19:51) armed on $(hostname) $(date '+%m/%d %H:%M') code=$(git -C /iris/u/kewalk/memory_project_v5 rev-parse --short HEAD)" >> $log
JOB=17267793 GPUS=1 BATCH=2 bash $cv5/run_train_h200.sh $acfg $aexp
