#!/usr/bin/env bash
# Run ON iris-hgx-1. Training chain only (user 16:12: start stage B right after A, no smoke; evals run on the H200):
...
echo "queue4 (A exit -> B r1 directly -> continuation; evals on the H200) armed on $(hostname) $(date '+%m/%d %H:%M') code=$(git -C /iris/u/kewalk/memory_project_v5 rev-parse --short HEAD)" >> $log
