#!/usr/bin/env bash
# Bank-geometry probe chain on the H200 job 17267793 (2026-09-05 18:25; scripts/v5_bank_geometry_eval.py).
...
log=$root/v5/diagnostics/bank_geometry_chain.log
echo "bank-geometry chain waiting for the H200 evals to drain $(date '+%m/%d %H:%M')" >> $log
while pgrep -f "v5_tray_flip_eva[l]|v5_heldout_vide[o]|v5_count_flip_eva[l]" >/dev/null; do sleep 60; done
