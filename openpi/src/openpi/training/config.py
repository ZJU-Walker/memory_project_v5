...
class DataConfig:
    ...
    # v5 generic task mode (cluster_v5/README.md §8, 2026-09-04, bean-scoop task): the sequence
    # loader uses a plain episode manifest (data_loader._load_v5_generic_manifest: stable ids,
    # splits, one class label per episode) and NO v3.5 phase machinery -- no left/right schema,
    # no waiting cores, no occlusion/execute phases, no dead zone. Windows are full trajectories,
    # random slices with the A5 history prefill, and (memory_critical_prob mass) slices starting
    # within memory_critical_start_pad frames before a sentence transition. The v3.5 per-step
    # fields the model expects are emitted with neutral values (transforms.MemoryV5GenericFields).
    memory_v5_generic_task: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if self.memory_v5_generic_task:
            if self.memory_v35_enabled or self.memory_v35_frozen_population:
                raise ValueError("memory_v5_generic_task replaces the v3.5 data machinery; disable memory_v35_enabled.")
            if self.memory_v5_subtask_labels_path is None or self.memory_v5_subtask_labels_sha256 is None:
                raise ValueError("memory_v5_generic_task requires the v5 sentence sidecar path and its pinned SHA256.")
            if self.memory_episode_manifest_path is None or self.memory_episode_manifest_sha256 is None:
                raise ValueError("memory_v5_generic_task requires the generic episode manifest path and its pinned SHA256.")
            if self.memory_manifest_split is None:
                raise ValueError("memory_v5_generic_task requires an explicit memory_manifest_split.")
            if self.memory_stride_frames <= 0 or self.subtask_lookahead < 0:
                raise ValueError("memory_v5_generic_task requires memory_stride_frames > 0 and subtask_lookahead >= 0.")
            if not self.subtask_from_task:
                raise ValueError("memory_v5_generic_task emits sentences through the sequence subtask transform (subtask_from_task).")
            if self.memory_v4_fact_labels_path is not None:
                raise ValueError("memory_v5_generic_task has no fact sidecar; the fact fields are emitted neutral.")
            if self.memory_waiting_max_speed is not None:
...
class LeRobotYamDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        use_v5_generic = use_memory and base_config.memory_v5_generic_task
        ...
        if use_v5_generic and not getattr(model_config, "memory_v4_dual_bank", False):
            raise ValueError("memory_v5_generic_task is a v5 dual-bank data mode (memory_v4_dual_bank).")
        if use_memory and not use_v5_generic and getattr(model_config, "memory_v4_dual_bank", False) != use_v4_facts:
            raise ValueError(
        ...
        if use_memory:
            ...
            if base_config.memory_required_subtasks and base_config.evidence_subtasks and not use_v5_generic:
                structure["memory_window"] = "memory_window"
            if use_v5_generic:
                structure["episode_memory_cell"] = "episode_memory_cell"
        if use_v4_facts:
        ...
        if use_v5_generic:
            # Runs after MemoryV34Labels (consumes its evidence/waiting masks); inserted first so
            # the v3.4 transform's insert(0) below lands in front of it.
            input_transforms.insert(
                0,
                _transforms.MemoryV5GenericFields(
                    num_fact_slots=model_config.memory_fact_slots,
                    num_fact_targets=model_config.memory_fact_targets,
                ),
            )
...
# v5 bean-scoop task (cluster_v5/BEANS_LABELS.md, README §8 2026-09-04): the 11 label sentences
# (= the LeRobot task strings of yam/bean_scoop_0902_v5) and their PaliGemma token ids with the
# trailing newline, exactly as FASTSubtaskTokenizer.tokenize_sentence produces them
# (generated 2026-09-04 with the paligemma tokenizer; the bins row reproduces bit-exactly).
V5_BEANS_SENTENCES: tuple[str, ...] = (
    "wait for the light: no green blink yet",
    "wait for the light: 1 green blink so far",
    "wait for the light: 2 green blinks so far",
    "wait for the light: 3 green blinks so far",
...
V5_BEANS_REFERENCE_SENTENCE_TOKENS: tuple[tuple[int, ...], ...] = (
    (9532, 604, 573, 2611, 235292, 793, 4433, 45741, 3599, 108),
    (9532, 604, 573, 2611, 235292, 235248, 235274, 4433, 45741, 712, 2166, 108),
    (9532, 604, 573, 2611, 235292, 235248, 235284, 4433, 212105, 712, 2166, 108),
    (9532, 604, 573, 2611, 235292, 235248, 235304, 4433, 212105, 712, 2166, 108),
...
# Steps whose observation lies in these phases are the beans "decision" steps (telemetry only):
# the go sentence must state the count from memory, and every scoop k>1 / done transition must be
# decided from the count and the progress so far.
V5_BEANS_EVIDENCE_SENTENCES: tuple[str, ...] = V5_BEANS_SENTENCES[1:4]
...
# v4 "tray-cut" scoops (README §8 2026-09-04 20:10, user "Ok do it"): the A keep_499 ORACLE videos showed the
# model copies the scoop counter from the bank and never detects the bowl-arrival boundary (with the v3 cut
# "previous = scoop k, arm over the bowl" is the input both during the dig of k and at the arrival for k+1:
# identical inputs, one boundary step per transition). `scoop k+1` now starts when the arm arrives OVER THE
# TRAY with scoop k (50-158 frames there, median 83 = ~17 memory steps; the return to the bowl is only 0-30
# frames), so the increment is decided in a persistent, visually distinct state; k == count keeps `scoop x`
# and `done` follows the last dump as before. Same 14 light-state sentences; only the sidecar changes
# (scripts/beans_relabel_scoop_dump.py --cut delivery_start -> beans_v5_subtask_labels_v4tray.json).
V5_BEANS_SIDECAR_V4_SHA256 = "728916ab22da300095550eeffde55dfc2a895d1a8b1aeb9097cc71a0505eef2c"
# v5 "visible LED" (2026-09-05 01:50): the LED control signal (led_on.npy) leads the camera by 0-2 frames (onset
# lag 1 frame in 73/100 blinks, 2 in 18), so ~1 in 5 stride-5 samples at a blink onset shows a dark LED with an
# "on" label; demo17's three onsets all sit on the rollout grid and every model lost a blink there. The light
# on/off boundaries of the tray-cut labels now sit on the first frame where the left camera shows the change
# (scripts/beans_relabel_visible_led.py; shifts 0/1/2 frames: 17/170/51 of 238 boundaries). Same 14 sentences.
...
# v4 "target-carry" sentences (README §8 2026-09-06 00:47; user: "when doing scoop we do 1 of x scoop, 2 of x
# scoop ..."): every scoop sentence carries the target x, so the tray decision (done vs "scoop k+1 of x") is a
# single-note copy/compare of the NEWEST note instead of a two-note comparison (go note + scoop note) that A6..A8
# never learned (tray-flip probe on A8: follows a flipped history 0/9, always "done"; count-flip 1.00 for the
# go count, which IS a single-note copy). 20 sentences, max 13 tokens (scripts/beans_relabel_target_carry.py on
# the 0905 raw folders -> beans_build_v5_manifest_sidecar.py --reuse-manifest, the 0905 v1 manifest). Token ids
# generated with the PaliGemma tokenizer exactly as FASTSubtaskTokenizer does (the V3 rows reproduce bit-exactly).
V5_BEANS_SENTENCES_V4: tuple[str, ...] = (
    "wait for the light: no green blink yet",
    "light on: 1 green blink so far",
    "light off: 1 green blink so far",
    "light on: 2 green blinks so far",
    ...
    "yellow go: pick up the scoop, scoop 2 times",
    "yellow go: pick up the scoop, scoop 3 times",
    "scoop 1 of 1: dig and carry",
    "scoop 1 of 2: dig and carry",
    "scoop 1 of 2: dump and return",
    "scoop 2 of 2: dig and carry",
    "scoop 1 of 3: dig and carry",
    "scoop 1 of 3: dump and return",
    "scoop 2 of 3: dig and carry",
    "scoop 2 of 3: dump and return",
    "scoop 3 of 3: dig and carry",
    "done, put down the scoop and return",
)
V5_BEANS_REFERENCE_SENTENCE_TOKENS_V4: tuple[tuple[int, ...], ...] = (
    (9532, 604, 573, 2611, 235292, 793, 4433, 45741, 3599, 108),  # wait for the light: no green blink yet
    (2462, 611, 235292, 235248, 235274, 4433, 45741, 712, 2166, 108),  # light on: 1 green blink so far
    (2462, 1401, 235292, 235248, 235274, 4433, 45741, 712, 2166, 108),  # light off: 1 green blink so far
    (2462, 611, 235292, 235248, 235284, 4433, 212105, 712, 2166, 108),  # light on: 2 green blinks so far
    (2462, 1401, 235292, 235248, 235284, 4433, 212105, 712, 2166, 108),  # light off: 2 green blinks so far
    (2462, 611, 235292, 235248, 235304, 4433, 212105, 712, 2166, 108),  # light on: 3 green blinks so far
    (2462, 1401, 235292, 235248, 235304, 4433, 212105, 712, 2166, 108),  # light off: 3 green blinks so far
    (22006, 871, 235292, 4788, 908, 573, 65522, 235269, 65522, 235248, 235274, 1069, 108),  # yellow go: pick up the scoop, scoop 1 time
    (22006, 871, 235292, 4788, 908, 573, 65522, 235269, 65522, 235248, 235284, 3023, 108),  # yellow go: pick up the scoop, scoop 2 times
    (22006, 871, 235292, 4788, 908, 573, 65522, 235269, 65522, 235248, 235304, 3023, 108),  # yellow go: pick up the scoop, scoop 3 times
    (17390, 715, 235248, 235274, 576, 235248, 235274, 235292, 3441, 578, 6383, 108),  # scoop 1 of 1: dig and carry
    (17390, 715, 235248, 235274, 576, 235248, 235284, 235292, 3441, 578, 6383, 108),  # scoop 1 of 2: dig and carry
    (17390, 715, 235248, 235274, 576, 235248, 235284, 235292, 21430, 578, 2203, 108),  # scoop 1 of 2: dump and return
    (17390, 715, 235248, 235284, 576, 235248, 235284, 235292, 3441, 578, 6383, 108),  # scoop 2 of 2: dig and carry
    (17390, 715, 235248, 235274, 576, 235248, 235304, 235292, 3441, 578, 6383, 108),  # scoop 1 of 3: dig and carry
    (17390, 715, 235248, 235274, 576, 235248, 235304, 235292, 21430, 578, 2203, 108),  # scoop 1 of 3: dump and return
    (17390, 715, 235248, 235284, 576, 235248, 235304, 235292, 3441, 578, 6383, 108),  # scoop 2 of 3: dig and carry
    (17390, 715, 235248, 235284, 576, 235248, 235304, 235292, 21430, 578, 2203, 108),  # scoop 2 of 3: dump and return
    (17390, 715, 235248, 235304, 576, 235248, 235304, 235292, 3441, 578, 6383, 108),  # scoop 3 of 3: dig and carry
    (7262, 235269, 2507, 1706, 573, 65522, 578, 2203, 108),  # done, put down the scoop and return
)
V5_BEANS_EVIDENCE_SENTENCES_V4: tuple[str, ...] = V5_BEANS_SENTENCES_V4[1:7]  # light on/off k
V5_BEANS_DECISION_SENTENCES_V4: tuple[str, ...] = V5_BEANS_SENTENCES_V4[7:10] + (
    "scoop 2 of 2: dig and carry", "scoop 2 of 3: dig and carry", "scoop 3 of 3: dig and carry",
    "done, put down the scoop and return",
)
# 0905 collection (89 episodes, 3 raw folders; other session 2026-09-05 23:35): manifest v1 (splits 2 dev + 2 final_test
# per class, train 23/30/24 for x=1/2/3) and the v7 target-carry sidecar pinned to it.
V5_BEANS_0905_MANIFEST_SHA256 = "3412223a9ab03ba0e63e544adae5efd94de0a551762c622661ba9f8983ae8776"
V5_BEANS_0905_SIDECAR_V7_SHA256 = "a2f704b948629709117aa5ee0ff55038ef0ff867fb5d9b6a46f31993c6b5826d"
...
_CONFIGS = [
    ...
    *(
        lambda v4_model=pi0_config.Pi0Config(
            ...
            # ---------------------------------------------------------------------------
            *(
                lambda v5_model, v5_data, v5_freeze_semantic_only, v5_freeze_dual, v5_loader, v5_beans_data, v5_beans_light_data, v5_beans_tray_data, v5_beans_vis_data, v5_beans_sub_data, v5_beans_0905_tgt_data: (
                    ...
                    # B5a (user 2026-09-03 20:20: "go back to two stages") = the A5 model warm-started from A5 r1
                    # ckpt-999 (every leaf, bf16 base leaves widened to f32 by the audited loader), the model's OWN
                    # delayed confidence-gated sentences, half the learning rate so self-writes only harden the
                    # reading/phase tracking learned on the clean oracle bank.
                    TrainConfig(
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_stageA5/v5_stageA5_20260903_r1/999/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # A6 (README §8, 2026-09-03 23:05 probe: the read queries were identical across frames, images
                    # and INSTRUCTIONS) = A5 + standardized instruction rows before the query conditioner + the
                    # previous decoded sentence shifting the base queries (zero-init). Warm start from A5 r1 ckpt-999
                    # (every leaf matched, only the new query-shift projection fresh), label writes.
                    TrainConfig(
                        name="pi05_yam_mem_v5_stageA6",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            ...
                            matched_allowlist=(r"(?!.*memory_sem_(?:prev|inst)_query_proj).+",),
                            fresh_init_allowlist=(r".*memory_sem_(?:prev|inst)_query_proj.*",),
                            source_cast_dtype="float32",
                        ),
                        v4_graft_sources=(),
                    ...
                    # Bean-scoop task, stage A (README §8 2026-09-04): the B6a architecture (A6 read queries,
                    # prefill, delay 1, exact writes) with LABEL writes on the beans sentences, warm-started
                    # from the judged B6a keep_499 (every leaf; the encoder/query standardization statistics
                    # follow the beans reference sentences at load time). prefill_max 10: up to 9 distinct
                    # sentences precede a window. 500 updates at global batch 8 on the 4xH100.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                            ...
                            memory_v5_pooling="standardized_attention",
                            memory_v5_pool_queries=4,
                            memory_v5_reference_tokens=V5_BEANS_REFERENCE_SENTENCE_TOKENS,
                            memory_v5_write_delay_steps=1,
                            memory_v5_prefill_history=True,
                            memory_v5_prefill_max=10,
                            memory_v5_query_standardize=True,
                            memory_v5_query_prev_sentence=True,
                        ),
                        data=v5_beans_data,
                    ...
                    # Bean-scoop stage B = beans-A weights (ckpt-499), the model's OWN delayed confidence-gated
                    # sentences, half the learning rate (the A -> B pattern of B5a/B6a).
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA/v5_beansA_20260904_r1/499/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # Bean-scoop A3 = beansA2 on the v4 TRAY-CUT scoop labels (V5_BEANS_SIDECAR_V4_SHA256; user 20:10 "Ok do it").
                    # Same warm start (B6a keep_499), lr, 500 updates; prefill_max 14: up to 13 distinct sentences
                    # precede a window (1 no-blink + 6 light on/off + go + scoops + done).
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA3",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                    ...
                    # Bean-scoop B3 = beansA3 weights (ckpt-499), the model's OWN delayed confidence-gated light-state
                    # sentences, half the learning rate.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB3",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA3/v5_beansA3_20260904_r1/499/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # Bean-scoop A4 = beansA3 on the v5 VISIBLE-LED labels (V5_BEANS_SIDECAR_V5_SHA256, 2026-09-05).
                    # Same warm start (B6a keep_499), lr, 500 updates; prefill_max 14: up to 13 distinct sentences
                    # precede a window (1 no-blink + 6 light on/off + go + scoops + done).
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA4",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                    ...
                    # Bean-scoop B4 = beansA4 weights (ckpt-499), the model's OWN delayed confidence-gated light-state
                    # sentences, half the learning rate.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB4",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA4/v5_beansA4_20260905_r1/499/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # Bean-scoop B5d1 = B5 with the one-step write delay back (control, 2026-09-05 13:20): B5 (delay 0 + own
                    # writes) collapses in rollouts (light off -> "no blink yet") although its teacher-forced telemetry is fine;
                    # A5 weights, own writes, retry rule, delay 1., the model's OWN delayed confidence-gated light-state
                    # sentences, half the learning rate.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB5d1",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                    ...
                    # Bean-scoop A6 = beansA5 on the v6 SUB-PHASE scoop sentences (V5_BEANS_SENTENCES_V3, 16 sentences; prefill_max
                    # 16). Delay 0 as A5 (lookahead 0: the label at
                    # step t describes the current observation, so writing it at t (after the read) leaks nothing, and the bank
                    # holds "light on: k" at the first "off" step even when a blink covers a single sampling step (demo17).
                    # Same warm start (B6a keep_499), lr, 500 updates; prefill_max 14: up to 13 distinct sentences
                    # precede a window (1 no-blink + 6 light on/off + go + scoops + done).
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA6",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                    ...
                    # Bean-scoop B6 = beansA6 weights (ckpt-499), delay 0, retry rule, the model's OWN delayed confidence-gated light-state
                    # sentences, half the learning rate.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB6",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA6/v5_beansA6_20260905_r1/499/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA6sd",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                            memory=dataclasses.replace(v5_model.memory, alpha_step=0.001),
                            memory_semantic=dataclasses.replace(v5_model.memory_semantic, alpha_step=0.001),
                            memory_v4_visual_injection=False,
                            memory_v5_pooling="standardized_attention",
                            memory_v5_pool_queries=4,
                            memory_v5_reference_tokens=V5_BEANS_REFERENCE_SENTENCE_TOKENS_V3,
                        ...
                        v4_graft_sources=(),
                        num_train_steps=300,  # user 17:03: 300 updates on the 4xH100 to save time
                        save_interval=250,
                        keep_period=250,
                        num_workers=12,
                        fsdp_devices=1,
                    ),
                    # Bean-scoop B6sd = beansA6sd weights (ckpt-499), slow decay 0.001, delay 0, retry rule, own sentences, half lr.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB6sd",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA6sd/v5_beansA6sd_20260905_r1/299/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # Bean-scoop A8 = beansA6 with SLOT KEYS + WHITENED VALUES (README §8 2026-09-05 20:40; bank replay: the go
                    # note's count survives as a 0.002-cosine residual -> readout 8/12). Same labels, delay 0, 300 updates.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA8",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                            memory_v5_slot_keys=True,
                            memory_v5_whiten_values=True,
                            memory_v4_visual_injection=False,
                            memory_v5_pooling="standardized_attention",
                            memory_v5_pool_queries=4,
                            memory_v5_reference_tokens=V5_BEANS_REFERENCE_SENTENCE_TOKENS_V3,
                    ...
                    # Bean-scoop B8 = beansA8 weights (ckpt-299), slot keys + whitened values, own writes, retry, delay 0.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB8",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA8/v5_beansA8_20260905_r1/299/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # Bean-scoop A9 = the A8 recipe (slot keys + whitened values, oracle writes, delay 0, 300 updates) on the
                    # v4 TARGET-CARRY sentences ("scoop k of x") and the 0905 collection only (user 2026-09-06 00:47).
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA9",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=True,
                            ...
                            memory_v5_pooling="standardized_attention",
                            memory_v5_pool_queries=4,
                            memory_v5_reference_tokens=V5_BEANS_REFERENCE_SENTENCE_TOKENS_V4,
                            memory_v5_write_delay_steps=0,
                            memory_v5_prefill_history=True,
                            memory_v5_prefill_max=16,
                            memory_v5_query_standardize=True,
                    ...
                    # Bean-scoop B9 = beansA9 weights (ckpt-299), own writes, retry, delay 0; v4 sentences, 0905 data.
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansB9",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            memory_v5_oracle_writes=False,
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA9/v5_beansA9_20260906_r1/299/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                            fresh_init_allowlist=(),
                    ...
                    # ckpt-299 (experiment name via OPENPI_V5_BEANS_A10_EXP, default v5_beansA10_20260907_r1).
                    TrainConfig(
                        name="pi05_yam_mem_v5_beansA10",
                        v4_protocol=True,
                        model=dataclasses.replace(
                            v5_model,
                            simulated_delay=15,
                    ...
                    TrainConfig(
                        ...
                        weight_loader=weight_loaders.AuditedPartialCheckpointWeightLoader(
                            str(
                                _project_paths.project_path(
                                    _project_paths.V5_CHECKPOINTS_DIR
                                    / "pi05_yam_mem_v5_beansA10"
                                    / os.environ.get("OPENPI_V5_BEANS_A10_EXP", "v5_beansA10_20260907_r1")
                                    / "299/params"
                                )
                            ),
                            matched_allowlist=(r".+",),
                ...
                # v5 bean-scoop task (README §8 2026-09-04): generic task data mode. Stride 5 (blinks last
                # 6-9 frames), lookahead 0 (a blink onset cannot be anticipated), no v3.5 phase machinery,
                # transition-anchored slices at half the mass, class-balanced over the target count.
                v5_beans_data=dataclasses.replace(
                    v4_data,
                    ...
                    base_config=dataclasses.replace(
                        ...
                        memory_critical_prob=0.5,
                        memory_critical_start_pad=75,
                        memory_subtask_vocab=V5_BEANS_SENTENCES,
                        heldout_episodes=(),
                ...
                v5_beans_tray_data=dataclasses.replace(
                    v4_data,
                    repo_id="yam/bean_scoop_0902_v5",
                    base_config=dataclasses.replace(
                        v4_data.base_config,
                        ...
                        memory_v4_fact_labels_sha256=None,
                        memory_v5_subtask_labels_path=str(
                            _project_paths.project_path("openpi/cluster_v5/beans/beans_v5_subtask_labels_v4tray.json")
                        ),
                        memory_v5_subtask_labels_sha256=V5_BEANS_SIDECAR_V4_SHA256,
                        memory_v5_generic_task=True,
                        lerobot_dataset_root=str(_project_paths.project_path("v5/data/lerobot/yam/bean_scoop_0902_v5")),
                    ),
                    assets=AssetsConfig(
                ...
                v5_beans_vis_data=dataclasses.replace(
                    v4_data,
                    repo_id="yam/bean_scoop_0902_v5",
                    base_config=dataclasses.replace(
                        v4_data.base_config,
                        ...
                        memory_v4_fact_labels_sha256=None,
                        memory_v5_subtask_labels_path=str(
                            _project_paths.project_path("openpi/cluster_v5/beans/beans_v5_subtask_labels_v5vis.json")
                        ),
                        memory_v5_subtask_labels_sha256=V5_BEANS_SIDECAR_V5_SHA256,
                        memory_v5_generic_task=True,
                        lerobot_dataset_root=str(_project_paths.project_path("v5/data/lerobot/yam/bean_scoop_0902_v5")),
                    ),
                    assets=AssetsConfig(
                ...
                v5_beans_sub_data=dataclasses.replace(
                    v4_data,
                    repo_id="yam/bean_scoop_0902_v5",
                    base_config=dataclasses.replace(
                        v4_data.base_config,
                        ...
                        memory_critical_prob=0.5,
                        memory_critical_start_pad=75,
                        memory_subtask_vocab=V5_BEANS_SENTENCES_V3,
                        heldout_episodes=(),
                        memory_waiting_max_speed=None,
                        memory_v35_enabled=False,
                        memory_e_tail_guard_frames=0,
                        ...
                        memory_v4_fact_labels_sha256=None,
                        memory_v5_subtask_labels_path=str(
                            _project_paths.project_path("openpi/cluster_v5/beans/beans_v5_subtask_labels_v6sub.json")
                        ),
                        memory_v5_subtask_labels_sha256=V5_BEANS_SIDECAR_V6_SHA256,
                        memory_v5_generic_task=True,
                        lerobot_dataset_root=str(_project_paths.project_path("v5/data/lerobot/yam/bean_scoop_0902_v5")),
                    ),
                    assets=AssetsConfig(
                ...
                # 0905 collection + v4 target-carry sentences (2026-09-06 00:47): A9/B9. Same loader settings as v5_beans_sub_data.
                v5_beans_0905_tgt_data=dataclasses.replace(
                    v4_data,
                    repo_id="yam/bean_scoop_0905_v5",
                    base_config=dataclasses.replace(
                        v4_data.base_config,
                        ...
                        memory_min_slice_steps=14,
                        memory_sequence_buckets=(14, 27, 40),
                        evidence_subtasks=V5_BEANS_EVIDENCE_SENTENCES_V4,
                        memory_required_subtasks=V5_BEANS_DECISION_SENTENCES_V4,
                        memory_critical_prob=0.5,
                        memory_critical_start_pad=75,
                        memory_subtask_vocab=V5_BEANS_SENTENCES_V4,
                        heldout_episodes=(),
                        memory_waiting_max_speed=None,
                        memory_v35_enabled=False,
                        memory_e_tail_guard_frames=0,
                        ...
                        memory_v5_generic_task=True,
                        lerobot_dataset_root=str(_project_paths.project_path("v5/data/lerobot/yam/bean_scoop_0905_v5")),
                    ),
                    assets=AssetsConfig(
                        assets_dir=str(_project_paths.project_path(_project_paths.V5_ASSETS_ROOT / "pi05_yam_bean_scoop_0905_v5"))
                    ),
                ),
            ),
        )
